        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="flxy_opt_btn text-center">
                    ...
                    <button type="button" class="btn btn-primary profile-notes" data_sysid=""
                        data_custname="">Notes</button>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="customerNotesWindow" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="customerNotesWindowLabel">Notes</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-lable="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="table-responsive text-nowrap">
                        <table id="customer_notes"
                            class="dt-responsive table table-striped table-bordered display nowrap">
                            <thead>
                                <tr>
                                    <th></th>
...
function showCustOptions(custOptId, custName) {
    ...
    $('#custOptionsWindow').find(
            '.data-port,.delete-record,.show-cust-activity-log,.show-cust-memberships,.show-cust-negotiated-rates,.merge-profile,.show-cust-preferences,.profile-notes'
        )
        .attr({
...
$(document).on('click', '.profile-notes', function() {

var custOptId = $(this).attr('data_sysid');
var custName = $(this).attr('data_custname');

$('#customerNotesWindow').modal('show');
showCustomerNotes(custOptId);

$('#customerNotesWindowLabel').html('Notes: ' + custName);

});

//Show notes table in modal

function showCustomerNotes(custId = 0) {

$('#customer_notes').DataTable({
    'processing': true,   
    'serverSide': true,
    'serverMethod': 'post',
    'ajax': {
        'url': '<?php echo base_url('/customerNotesView')?>',
        'data': {
            "sysid": custId
        }
    },
    ...
    language: {
        emptyTable: 'There are no notes for this customer'
    },
    responsive: {
        details: {
            display: $.fn.dataTable.Responsive.display.modal({
                header: function(row) {
                    var data = row.data();
                    return 'Notes of Customer Profile';
                }
            }),
            type: 'column',
