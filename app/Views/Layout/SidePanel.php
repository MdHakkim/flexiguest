        <!-- Menu -->

        <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
            <div class="app-brand demo">
                <a href="<?php echo base_url('/') ?>" class="app-brand-link">
                    <span class="app-brand-logo demo">

                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 187.6 228.8"
                            style="enable-background:new 0 0 187.6 228.8;" xml:space="preserve">
                            <style type="text/css">
                            .st0 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: #18371C;
                            }

                            .st1 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: url(#SVGID_1_);
                            }

                            .st2 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: url(#SVGID_00000157279095315231982060000005211230105190083231_);
                            }

                            .st3 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: url(#SVGID_00000012451031522960550390000012626224618914400140_);
                            }

                            .st4 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: url(#SVGID_00000016070205341026246050000000039972798898783923_);
                            }

                            .st5 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: url(#SVGID_00000052805987559678658880000016781226765522586761_);
                            }

                            .st6 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: url(#SVGID_00000151513121863170215690000006295043811959257524_);
                            }

                            .st7 {
                                opacity: 0.15;
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: #161920;
                                enable-background: new;
                            }

                            .st8 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: url(#SVGID_00000043448609969055518270000012130186912031525559_);
                            }

                            .st9 {
                                fill-rule: evenodd;
                                clip-rule: evenodd;
                                fill: url(#SVGID_00000060011834154892075660000005155630478817162145_);
                            }
                            </style>
                            <path class="st0"
                                d="M4.9,126.3c0,37.8,93.7,55.2,120.3,11l23-47.2C140.6,105.6,24.2,155.9,5,61L4.9,97.4V126.3z" />
                            <linearGradient id="SVGID_1_" gradientUnits="userSpaceOnUse" x1="185.8746" y1="238.7527"
                                x2="-43.7753" y2="111.4127" gradientTransform="matrix(1 0 0 -1 0 222.6383)">
                                <stop offset="0" style="stop-color:#E7E754" />
                                <stop offset="1" style="stop-color:#5CB431" />
                            </linearGradient>
                            <path class="st1"
                                d="M90.4,57.8h44.8c28.8,0,49.2-36,49.2-54H86.9C44.2,3.8,4.8,25.9,4.8,65.1v61.2C10.5,82.7,47.7,57.8,90.4,57.8z" />
                            <linearGradient id="SVGID_00000001665901154226395070000017377122798177628309_"
                                gradientUnits="userSpaceOnUse" x1="177.4547" y1="227.0634" x2="-62.9653" y2="74.5734"
                                gradientTransform="matrix(1 0 0 -1 0 222.6383)">
                                <stop offset="0" style="stop-color:#E7E754" />
                                <stop offset="1" style="stop-color:#2B893A" />
                            </linearGradient>
                            <path
                                style="fill-rule:evenodd;clip-rule:evenodd;fill:url(#SVGID_00000001665901154226395070000017377122798177628309_);"
                                d="
	M6.6,76.9v33.8c11.7-35.1,45.8-54.6,83.8-54.6h44.8c3.9,0,7.8-0.7,11.4-2c7.5-2.7,14-7.8,19.3-13.6c5.1-5.7,9.2-12.1,12.3-19.1
	c1.9-4.3,3.3-8.8,4.1-13.4c-1.8,2.6-3.7,5.1-5.8,7.5c-5.3,5.9-11.8,10.9-19.3,13.6c-3.7,1.3-7.5,2-11.4,2c-14.9,0-44.3,0.9-59.3,0.9
	C52.1,31.9,20.9,47.8,6.6,76.9z" />
                            <linearGradient id="SVGID_00000142161053142947608910000017353340766530899897_"
                                gradientUnits="userSpaceOnUse" x1="214.0623" y1="185.2717" x2="-12.5577" y2="36.1917"
                                gradientTransform="matrix(1 0 0 -1 0 222.6383)">
                                <stop offset="0" style="stop-color:#E7E754" />
                                <stop offset="1" style="stop-color:#5CB431" />
                            </linearGradient>
                            <path
                                style="fill-rule:evenodd;clip-rule:evenodd;fill:url(#SVGID_00000142161053142947608910000017353340766530899897_);"
                                d="
	M86.1,137.3h27.6c2.3,0,10.5,0.5,6.7,6.6c1.8-2,3.5-4.2,4.8-6.6l22.9-47c3.2-6.6-4.8-7-7.7-7H82.5C39.7,83.5,5,118.2,4.9,161v63.3
	c16.8,0,43.5-20.1,43.5-41.8C48.3,161.6,42,137.3,86.1,137.3z" />
                            <linearGradient id="SVGID_00000026845750694380275960000014168185075937240968_"
                                gradientUnits="userSpaceOnUse" x1="178.1774" y1="166.3042" x2="17.2574" y2="47.4942"
                                gradientTransform="matrix(1 0 0 -1 0 222.6383)">
                                <stop offset="0" style="stop-color:#E7E754" />
                                <stop offset="1" style="stop-color:#2B893A" />
                            </linearGradient>
                            <path
                                style="fill-rule:evenodd;clip-rule:evenodd;fill:url(#SVGID_00000026845750694380275960000014168185075937240968_);"
                                d="
	M122.3,138.1c0.4-0.5,0.7-1.1,1-1.7l11.7-24.1c-15.2-0.2-44-0.8-51.6-0.8c-4.2,0-8.5,0.2-12.7,0.8l0,0l-2.1,0.3
	c-25.2,4-48.5,14.3-60.4,34.1c-0.9,4.7-1.4,9.5-1.4,14.3v17.6c9.9-25.1,35.9-37.7,64.4-42.2l2.1-0.3l0,0c4.2-0.6,8.5-0.8,12.7-0.8
	h27.6c1.5,0,3,0.2,4.5,0.5C119.7,136,121.2,136.9,122.3,138.1z" />
                            <linearGradient id="SVGID_00000110460869155740343570000017850948155766471613_"
                                gradientUnits="userSpaceOnUse" x1="35.4944" y1="-52.6618" x2="42.5244" y2="115.9482"
                                gradientTransform="matrix(1 0 0 -1 0 222.6383)">
                                <stop offset="0" style="stop-color:#E7E754" />
                                <stop offset="1" style="stop-color:#2B893A" />
                            </linearGradient>
                            <path
                                style="fill-rule:evenodd;clip-rule:evenodd;fill:url(#SVGID_00000110460869155740343570000017850948155766471613_);"
                                d="
	M4.9,202v22.3c16.8,0,43.5-20.1,43.5-41.8c0-18.3-4.8-39.1,23.3-44.1C51.6,141.7,4.4,153.3,4.9,202L4.9,202z M75,137.9L75,137.9z" />
                            <linearGradient id="SVGID_00000182513730796094447060000010748656252746595768_"
                                gradientUnits="userSpaceOnUse" x1="14.5765" y1="-41.5845" x2="25.5865" y2="96.7855"
                                gradientTransform="matrix(1 0 0 -1 0 222.6383)">
                                <stop offset="0" style="stop-color:#E7E754" />
                                <stop offset="1" style="stop-color:#5CB431" />
                            </linearGradient>
                            <path
                                style="fill-rule:evenodd;clip-rule:evenodd;fill:url(#SVGID_00000182513730796094447060000010748656252746595768_);"
                                d="
	M6.5,222.6c6.1-0.5,12.7-3.4,18.4-7c2.8-8.5,2-21.1,0.9-37.1s10.9-26.9,11-27C19,161.2,6.5,177.3,6.5,202.1L6.5,222.6z" />
                            <path class="st7" d="M49.9,154.2c2.6-7.5,8.5-13.5,21.8-15.8c-19.8,3.2-65.8,14.5-66.8,61.4C5.3,192.7,10,170.1,49.9,154.2
	L49.9,154.2z M75,137.9L75,137.9z" />
                            <linearGradient id="SVGID_00000005267255372703103320000011831359240645276035_"
                                gradientUnits="userSpaceOnUse" x1="1.1712" y1="109.9574" x2="184.1612" y2="208.8374"
                                gradientTransform="matrix(1 0 0 -1 0 222.6383)">
                                <stop offset="0" style="stop-color:#2B893A" />
                                <stop offset="0.22" style="stop-color:#2D8A3B" />
                                <stop offset="0.3" style="stop-color:#348E3F" />
                                <stop offset="0.36" style="stop-color:#3F9447" />
                                <stop offset="0.41" style="stop-color:#509D51" />
                                <stop offset="0.45" style="stop-color:#66A85F" />
                                <stop offset="0.48" style="stop-color:#82B76F" />
                                <stop offset="0.52" style="stop-color:#A2C884" />
                                <stop offset="0.55" style="stop-color:#C8DC9B" />
                                <stop offset="0.57" style="stop-color:#F1F2B4" />
                                <stop offset="0.57" style="stop-color:#F7F5B8" />
                                <stop offset="1" style="stop-color:#FDF160" />
                            </linearGradient>
                            <path
                                style="fill-rule:evenodd;clip-rule:evenodd;fill:url(#SVGID_00000005267255372703103320000011831359240645276035_);"
                                d="
	M178.2,21.3c-3.1,7-7.2,13.4-12.3,19.1c-5.3,5.9-11.8,10.9-19.3,13.6c-3.7,1.3-7.5,2-11.4,2H90.4c-30.1,0-57.7,12.2-73.7,34.6
	c-4.4,6.1-7.8,12.9-10.1,20v-0.3c-0.7,2.6-1.3,5.2-1.8,7.9v8.1c1.5-12.3,6-24,13-34.1C33.4,70,60.6,57.8,90.5,57.8h44.8
	c28.5,0,48.8-35.3,49.2-53.4l-1.8,1.4C182,11.1,180.5,16.4,178.2,21.3z" />
                            <linearGradient id="SVGID_00000099647703509215640730000017861532716248486312_"
                                gradientUnits="userSpaceOnUse" x1="0.5407" y1="39.9869" x2="117.6607" y2="90.8269"
                                gradientTransform="matrix(1 0 0 -1 0 222.6383)">
                                <stop offset="0" style="stop-color:#2B893A" />
                                <stop offset="0.14" style="stop-color:#46974B" />
                                <stop offset="0.43" style="stop-color:#8BBC75" />
                                <stop offset="0.86" style="stop-color:#F7F5B8" />
                                <stop offset="0.89" style="stop-color:#F7F5B4" />
                                <stop offset="0.92" style="stop-color:#F8F4A9" />
                                <stop offset="0.95" style="stop-color:#F9F396" />
                                <stop offset="0.98" style="stop-color:#FBF27B" />
                                <stop offset="1" style="stop-color:#FDF160" />
                            </linearGradient>
                            <path
                                style="fill-rule:evenodd;clip-rule:evenodd;fill:url(#SVGID_00000099647703509215640730000017861532716248486312_);"
                                d="
	M28,154.9c14.7-10.6,32.8-14.7,43.5-16.4h0.1c1.1-0.2,2.2-0.4,3.4-0.5l0,0c3.7-0.4,7.3-0.6,11-0.6h27.6c2.3,0,10.5,0.5,6.7,6.6l0,0
	c0.5-0.6,1.1-1.3,1.7-2s0.9-1.2,1.4-1.8c-0.2-0.8-0.5-1.5-0.9-2.2l-0.2,0.2c-1.1-1.2-2.6-2-4.2-2.3c-1.5-0.3-3-0.5-4.5-0.5H86.1
	c-4.2,0-8.5,0.2-12.7,0.8l0,0l-2.1,0.3c-16.9,2.7-32.9,8.2-45.3,17.5c-8.6,6.3-15.2,14.8-19.2,24.7v-0.2c-0.7,2.4-1.4,5-2.1,7.6
	v14.8C5,178.5,15.2,164.2,28,154.9z" />
                        </svg>

                    </span>
                    <span class="app-brand-text demo menu-text fw-bold ms-2">FlexiGuest</span>
                </a>

                <a href="javascript:void(0);" class="layout-menu-toggle menu-link text-large ms-auto">
                    <i class="bx menu-toggle-icon fs-4 d-none d-xl-block align-middle"></i>
                    <i class="bx bx-x bx-sm d-xl-none d-block align-middle"></i>
                </a>
            </div>

            <div class="menu-divider mt-0"></div>

            <div class="menu-inner-shadow"></div>

            <ul class="menu-inner py-1">
                <!-- Dashboards -->
                <li class="menu-item <?= (isset($title) && in_array($title, ['Dashboard'])) ? 'active' : '' ?>">
                    <a href="<?php echo base_url('/') ?>" class="menu-link">

                        <i class="menu-icon fas fa-tachometer-alt"></i>
                        <div data-i18n="Dashboard">Dashboard</div>
                    </a>
                </li>

                <!-- Layouts -->
                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">

                        <i class="menu-icon fas fa-sitemap"></i>
                        <div data-i18n="Organization">Organization</div>
                    </a>

                    <ul class="menu-sub">
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Business Units">Business Units</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Contacts">Contacts</div>
                            </a>
                        </li>

                    </ul>
                </li>

                <!-- Apps & Pages 
            <li class="menu-header small text-uppercase"><span class="menu-header-text">Apps &amp; Pages</span></li>
            <li class="menu-item">
              <a href="app-calendar.html" class="menu-link">
                <i class="menu-icon tf-icons bx bx-calendar"></i>
                <div data-i18n="Calendar">Calendar</div>
              </a>
            </li> -->
                <li class="menu-item <?= (isset($title) && in_array($title, ['Shuttle', 'Stages'])) ? 'open' : '' ?>">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">

                        <i class="menu-icon fas fa-hotel"></i>
                        <div data-i18n="Transfers">Transfers</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Guest Requests">Guest Requests</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="Config">Config</div>
                            </a>
                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Carriers">Carriers</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Destination Points">Destination Points</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Transport Type">Transport Type</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Transfer Purpose">Transfer Purpose</div>
                                    </a>
                                </li>
                            </ul>
                        </li>

                        <li
                            class="menu-item <?= (isset($title) && in_array($title, ['Shuttle', 'Stages'])) ? 'open' : '' ?>">

                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="Shuttle">Shuttle</div>
                            </a>

                            <ul class="menu-sub">

                                <li class="menu-item <?= (isset($title) && $title == 'Shuttle') ? 'active' : '' ?>">
                                    <a href="<?php echo base_url('shuttle'); ?>" class="menu-link">
                                        <div data-i18n="Routes">Routes</div>
                                    </a>
                                </li>

                                <li class="menu-item <?= (isset($title) && $title == 'Stages') ? 'active' : '' ?>">
                                    <a href="<?php echo base_url('stages'); ?>" class="menu-link">
                                        <div data-i18n="Stages">Stages</div>
                                    </a>
                                </li>
                            </ul>
                        </li>

                    </ul>
                </li>


                <li
                    class="menu-item <?= (isset($title) && in_array($title, ['Concierge Offer', 'Concierge Request'])) ? 'open' : '' ?>">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">
                        <i class="menu-icon fas fa-bahai"></i>

                        <div data-i18n="Concierge">Concierge</div>
                    </a>

                    <ul class="menu-sub">
                        <li class="menu-item <?= (isset($title) && $title == 'Concierge Offer') ? 'active' : '' ?>">
                            <a href="<?= base_url('concierge/concierge-offer') ?>" class="menu-link">
                                <div data-i18n="Concierge Offers">Concierge Offers</div>
                            </a>
                        </li>

                        <li class="menu-item <?= (isset($title) && $title == 'Concierge Request') ? 'active' : '' ?>">
                            <a href="<?= base_url('concierge/concierge-request') ?>" class="menu-link">
                                <div data-i18n="Concierge Request">Concierge Request</div>
                            </a>
                        </li>

                    </ul>
                </li>



                <li class="menu-item   <?php if (isset($title) && in_array($title, array(
                                            'Reservation',
                                            'All Profiles',
                                            'Customer',
                                            'Company',
                                            'Agent',
                                            'Group',
                                            'Block',
                                            'Room',
                                            'Room Class',
                                            'Room Type',
                                            'Room Floor',
                                            'Room Feature',
                                            'Section',
                                            'Source',
                                            'Source Group',
                                            'Special',
                                            'Reservation Type',
                                            'Purpose Stay',
                                            'Payment',
                                            'Over Booking',
                                            'Guest Registration Cards'
                                        ))) {
                                            echo 'open active';
                                        } ?>">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">
                        <i class="menu-icon fas fa-luggage-cart"></i>

                        <div data-i18n="Reservations">Reservations</div>
                    </a>

                    <ul class="menu-sub">
                        <li class="menu-item  <?php if (isset($title) && $title == 'Reservation') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('reservation'); ?>" class="menu-link">
                                <div data-i18n="Add / List Reservations">Add / List Reservations</div>
                            </a>
                        </li>

                        <li class="menu-item  <?php if (isset($title) && $title == 'Room Class') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('roomClass'); ?>" class="menu-link">
                                <div data-i18n="Room Class">Room Class</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Room Type') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('roomType'); ?>" class="menu-link">
                                <div data-i18n="Room Type">Room Type</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Room') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('room'); ?>" class="menu-link">
                                <div data-i18n="Rooms">Rooms</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Room Floor') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('roomFloor'); ?>" class="menu-link">
                                <div data-i18n="Room Floors">Room Floors</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Room Feature') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('roomFeature'); ?>" class="menu-link">
                                <div data-i18n="Room Features">Room Features</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'All Profiles') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('profiles'); ?>" class="menu-link">
                                <div data-i18n="All Profiles">All Profiles</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Customer') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('customer'); ?>" class="menu-link">
                                <div data-i18n="Customers">Customers</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Company') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('company'); ?>" class="menu-link">
                                <div data-i18n="Company">Company</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Agent') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('agent'); ?>" class="menu-link">
                                <div data-i18n="Agent">Agent</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Group') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('group'); ?>" class="menu-link">
                                <div data-i18n="Group">Group</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="<?php echo base_url('rateClass'); ?>" class="menu-link">
                                <div data-i18n="Class Rate">Class Rate</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Section') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('section'); ?>" class="menu-link">
                                <div data-i18n="Sections">Sections</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Source Group') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('sourceGroup'); ?>" class="menu-link">
                                <div data-i18n="Source Group">Source Group</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Source') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('source'); ?>" class="menu-link">
                                <div data-i18n="Sources">Sources</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Over Booking') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('overBooking'); ?>" class="menu-link">
                                <div data-i18n="OverBooking">OverBooking</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Special') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('special'); ?>" class="menu-link">
                                <div data-i18n="Specials">Specials</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Reservation Type') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('reservationType'); ?>" class="menu-link">
                                <div data-i18n="Reservation Type">Reservation Type</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Purpose Stay') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('purposeStay'); ?>" class="menu-link">
                                <div data-i18n="Purpose Of Stay">Purpose Of Stay</div>
                            </a>
                        </li>
                        <li class="menu-item <?php if (isset($title) && $title == 'Payment') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('payment'); ?>" class="menu-link">
                                <div data-i18n="Payment Types">Payment Types</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Search Arrivals">Search Arrivals</div>
                            </a>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Pre CheckInLink">Pre CheckInLink</div>
                            </a>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Sync Reservation">Sync Reservation</div>
                            </a>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Vaccine Assessment">Vaccine Assessment</div>
                            </a>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Vaccine Types">Vaccine Types</div>
                            </a>
                        </li>

                        <li class="menu-item  <?php if (isset($title) && $title == 'Guest Registration Cards') {
                                    echo 'active';
                                } ?>">
                            <a href="<?php echo base_url('registerCards'); ?>" class="menu-link">
                                <div data-i18n="Registration Cards">Registration Cards</div>
                            </a>
                        </li>


                    </ul>
                </li>


                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">
                        <i class="menu-icon fas fa-bed"></i>
                        <div data-i18n="Facility">Facility</div>
                    </a>

                    <ul class="menu-sub">
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="Amenities">Amenities</div>
                            </a>
                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Manage">Manage</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Wizard">Wizard</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Provider View">Provider View</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Stock Update">Stock Update</div>
                                    </a>
                                </li>

                            </ul>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="House Keeping">House Keeping</div>
                            </a>
                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Tasks">Tasks</div>
                                    </a>
                                </li>
                            </ul>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="Maintenance Request">Maintenance Request</div>
                            </a>
                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="<?php echo base_url('maintenance'); ?>" class="menu-link">
                                        <div data-i18n="Create Request">Create Request</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="<?php echo base_url('maintenanceCategory'); ?>" class="menu-link">
                                        <div data-i18n="Category">Category</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="<?php echo base_url('maintenanceSubCategory'); ?>" class="menu-link">
                                        <div data-i18n="Sub Category">Sub Category</div>
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="Config">Config</div>
                            </a>
                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Guest Request Status">Guest Request Status</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Articles">Articles</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Article Type">Article Type</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Departments">Departments</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Floors">Floors</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Room Class">Room Class</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Rooms">Rooms</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Room Sections">Room Sections</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Categories">Service Categories</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Configuration">Service Configuration</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Group">Service Group</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Group Members">Service Group Members</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Group Category">Service Group Category</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Item">Service Item</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Item Properties">Service Item Properties</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Item Type">Service Item Type</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Locations">Service Locations</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Member">Service Member</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Member Type">Service Member Type</div>
                                    </a>
                                </li>

                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Priority">Service Priority</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Properties">Service Properties</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Property Types">Service Property Types</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Status">Service Status</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Tags">Service Tags</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Task List Template">Task List Template</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Time Slots">Time Slots</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Reject Reasons">Reject Reasons</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Service Escalation Control">Service Escalation Control</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Buildings">Buildings</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Towers">Towers</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Location Asset Map">Location Asset Map</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Asset Category">Asset Category</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Asset Service Reasons">Asset Service Reasons</div>
                                    </a>
                                </li>
                            </ul>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="Assets Tracking">Assets Tracking</div>
                            </a>
                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Assets">Assets</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Types">Types</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Verifications">Verifications</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Assets Handover">Assets Handover</div>
                                    </a>
                                </li>
                            </ul>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Escalation Category">Escalation Category</div>
                            </a>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Escalation Interval Config">Escalation Interval Config</div>
                            </a>
                        </li>



                    </ul>

                </li>




                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">
                        <i class="menu-icon fas fa-user-lock"></i>

                        <div data-i18n="Administration">Administration</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Exceptions Log">Exceptions Log</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Mail Queue">Mail Queue</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Languages">Languages</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Translations">Translations</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Roles">Roles</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="User Management">User Management</div>
                            </a>
                        </li>
                    </ul>
                </li>


                <li class="menu-item  <?php if (isset($title) && in_array($title, array(
                                            'Rate Class',
                                            'Rate Category',
                                            'Rate Code',
                                            'Add Rate Code',
                                            'Edit Rate Code',
                                            'Transaction Code Group',
                                            'Transaction Code Sub Group',
                                            'Transaction Code',
                                            'Package Group',
                                            'Package Code',
                                            'Market Group',
                                            'Market Code',
                                            'News',
                                            'Guideline',
                                            'App Update',
                                            'Transport',
                                            'Currency',
                                            'Exchange Codes',
                                            'Exchange Rates',
                                            'Item Class',
                                            'Items',
                                            'Daily Inventory',
                                            'Membership Type', 
                                            'Menu', 
                                            'Sub Menu'                                   
                                        ))) {
                                            echo 'open active';
                                        } ?>">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">
                        <i class="menu-icon fas fa-user-cog"></i>
                        <div data-i18n="Masters">Masters</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item  <?php if (isset($title) && $title == 'Rate Class') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('rateClass') ?>" class="menu-link">
                                <div data-i18n="Rate Classes">Rate Classes</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Rate Category') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('rateCategory') ?>" class="menu-link">
                                <div data-i18n="Rate Categories">Rate Categories</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && in_array($title, array(
                                                    'Rate Code',
                                                    'Add Rate Code',
                                                    'Edit Rate Code'
                                                ))) {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('rateCode') ?>" class="menu-link">
                                <div data-i18n="Rate Codes">Rate Codes</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Transaction Code Group') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('transactionCodeGroup') ?>" class="menu-link">
                                <div data-i18n="Transaction Code Groups">Transaction Code Groups</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Transaction Code Sub Group') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('transactionCodeSubGroup') ?>" class="menu-link">
                                <div data-i18n="Transaction Code Sub Groups">Transaction Code Sub Groups</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Transaction Code') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('transactionCode') ?>" class="menu-link">
                                <div data-i18n="Transaction Codes">Transaction Codes</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Package Group') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('packageGroup') ?>" class="menu-link">
                                <div data-i18n="Package Groups">Package Groups</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Package Code') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('packageCode') ?>" class="menu-link">
                                <div data-i18n="Package Codes">Package Codes</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Market Group') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('marketGroup') ?>" class="menu-link">
                                <div data-i18n="Market Groups">Market Groups</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Market Code') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('marketCode') ?>" class="menu-link">
                                <div data-i18n="Market Codes">Market Codes</div>
                            </a>
                        </li>
                        <li class="menu-item  <?php if (isset($title) && $title == 'Membership Type') {
                                                    echo 'active';
                                                } ?>">
                            <a href="<?php echo base_url('membershipType') ?>" class="menu-link">
                                <div data-i18n="Membership Types">Membership Types</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Master Property Types">Master Property Types</div>
                            </a>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Guest Preferences">Guest Preferences</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Master Properties">Master Properties</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Profile Preferences">Profile Preferences</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Address Types">Address Types</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Document Types">Document Types</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Profile Types">Profile Types</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Room Types">Room Types</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Guest Action Categories">Guest Action Categories</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Email Templates">Email Templates</div>
                            </a>
                        </li>

                        <li class="menu-item <?= (isset($title) && $title == 'News') ? 'active' : '' ?>">
                            <a href="<?= base_url('/news') ?>" class="menu-link">
                                <div data-i18n="News">News</div>
                            </a>
                        </li>

                        <li class="menu-item <?= (isset($title) && $title == 'Guideline') ? 'active' : '' ?>">
                            <a href="<?= base_url('/guideline') ?>" class="menu-link">
                                <div data-i18n="Guidelines">Guidelines</div>
                            </a>
                        </li>

                        <li class="menu-item <?= (isset($title) && $title == 'App Update') ? 'active' : '' ?>">
                            <a href="<?= base_url('/app-update') ?>" class="menu-link">
                                <div data-i18n="App Update">App Update</div>
                            </a>
                        </li>

                        <li class="menu-item <?= (isset($title) && $title == 'Transport') ? 'active' : '' ?>">
                            <a href="<?= base_url('/transport') ?>" class="menu-link">
                                <div data-i18n="Transport">Transport</div>
                            </a>
                        </li>

                        <li class="menu-item <?= (isset($title) && $title == 'Currency') ? 'active' : '' ?>">
                            <a href="<?= base_url('/currency') ?>" class="menu-link">
                                <div data-i18n="Currency">Currency</div>
                            </a>
                        </li>
                        <li class="menu-item <?= (isset($title) && $title == 'Exchange Codes') ? 'active' : '' ?>">
                            <a href="<?= base_url('/exchangeCodes') ?>" class="menu-link">
                                <div data-i18n="Exchange Codes">Exchange Codes</div>
                            </a>
                        </li>
                        <li class="menu-item <?= (isset($title) && $title == 'Exchange Rates') ? 'active' : '' ?>">
                            <a href="<?= base_url('/exchangeRates') ?>" class="menu-link">
                                <div data-i18n="Exchange Rates">Exchange Rates</div>
                            </a>
                        </li>
                        <li class="menu-item <?= (isset($title) && $title == 'Item Class') ? 'active' : '' ?>">
                            <a href="<?= base_url('/itemClass') ?>" class="menu-link">
                                <div data-i18n="Item Class">Item Class</div>
                            </a>
                        </li>
                        <li class="menu-item <?= (isset($title) && $title == 'Items') ? 'active' : '' ?>">
                            <a href="<?= base_url('/items') ?>" class="menu-link">
                                <div data-i18n="Items">Items</div>
                            </a>
                        </li>
                        <li class="menu-item <?= (isset($title) && $title == 'Daily Inventory') ? 'active' : '' ?>">
                            <a href="<?= base_url('/dailyInventory') ?>" class="menu-link">
                                <div data-i18n="Daily Inventory">Daily Inventory</div>
                            </a>
                        </li>

                        <li class="menu-item <?= (isset($title) && $title == 'Menu') ? 'active' : '' ?>">
                            <a href="<?= base_url('/Menu') ?>" class="menu-link">
                                <div data-i18n="Menu">Menu</div>
                            </a>
                        </li>

                        <li class="menu-item <?= (isset($title) && $title == 'Sub Menu') ? 'active' : '' ?>">
                            <a href="<?= base_url('SubMenu') ?>" class="menu-link">
                                <div data-i18n="Sub Menu">Sub Menu</div>
                            </a>
                        </li>

                    </ul>
                </li>

                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">

                        <i class="menu-icon fas fa-tools"></i>
                        <div data-i18n="Config">Config</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Package Config">Package Config</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="System Settings">System Settings</div>
                            </a>
                        </li>

                    </ul>
                </li>

                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link">

                        <i class="menu-icon fas fa-paste"></i>
                        <div data-i18n="Reports">Reports</div>
                    </a>
                </li>


                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">

                        <i class="menu-icon fas fa-poll"></i>
                        <div data-i18n="Survey">Survey</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="Setup">Setup</div>
                            </a>

                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Answer Types">Answer Types</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Answer Options">Answer Options</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Survey Category">Survey Category</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Survey">Survey</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Questions">Questions</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Emoticons">Emoticons</div>
                                    </a>
                                </li>
                            </ul>

                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Health Ass.Review">Health Ass.Review</div>
                            </a>
                        </li>

                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <div data-i18n="Analysis">Analysis</div>
                            </a>
                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Survey Stats">Survey Stats</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Responses-Summary">Responses-Summary</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="menu-link">
                                        <div data-i18n="Responses-Details">Responses-Details</div>
                                    </a>
                                </li>

                            </ul>
                        </li>

                        <li class="menu-item">
                            <a href="<?php echo base_url('feedback'); ?>" class="menu-link">
                                <div data-i18n="General Feedback">General Feedback</div>
                            </a>
                        </li>

                    </ul>
                </li>



                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">

                        <i class="menu-icon fas fa-clipboard-list"></i>
                        <div data-i18n="Logs">Logs</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Audit Log">Audit Log</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="interface Log">interface Log</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Data Audit Log">Data Audit Log</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="System Logs">System Logs</div>
                            </a>
                        </li>
                    </ul>
                </li>


                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link menu-toggle">

                        <i class="menu-icon fas fa-newspaper"></i>
                        <div data-i18n="News & Notifications">News & Notifications</div>
                    </a>
                    <ul class="menu-sub">
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Posts">Posts</div>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a href="javascript:void(0);" class="menu-link">
                                <div data-i18n="Content Manager">Content Manager</div>
                            </a>
                        </li>

                    </ul>
                </li>


                <!-- Misc -->
                <li class="menu-header small text-uppercase"><span class="menu-header-text">Misc</span></li>
                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link">
                        <i class="menu-icon tf-icons bx bx-support"></i>
                        <div data-i18n="Support">Support</div>
                    </a>
                </li>
                <li class="menu-item">
                    <a href="javascript:void(0);" class="menu-link">
                        <i class="menu-icon tf-icons bx bx-file"></i>
                        <div data-i18n="Documentation">Documentation</div>
                    </a>
                </li>
            </ul>
        </aside>
        <!-- / Menu -->