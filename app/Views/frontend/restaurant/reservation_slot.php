...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="breadcrumb-wrapper py-3 mb-4"><span class="text-muted fw-light">Restaurant /</span> Reservation Slots</h4>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="submit-form" class="needs-validation" novalidate>
                        <div class="row g-3">
                            <input type="hidden" name="RRS_ID" class="form-control" />

                            <div class="col-md-4">
                                <label class="form-label"><b>From Time *</b></label>
                                <input type="time" name="RRS_FROM_TIME" class="form-control" />
                            </div>

                            <div class="col-md-4">
                                <label class="form-label"><b>To Time *</b></label>
                                <input type="time" name="RRS_TO_TIME" class="form-control" />
                            </div>

                            <div class="col-md-4">
                                <label class="form-label"><b>Display Sequence</b></label>
                                <input type="number" name="RRS_DISPLAY_SEQUENCE" class="form-control" value="1" required />
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#dataTable_view').DataTable({
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': '<?php echo base_url('/restaurant/reservation-slot/all-reservation-slots') ?>'
            },
            'columns': [{
                    data: ''
                },
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        <div class="d-inline-block">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    <a href="javascript:;" data_id="${data['RRS_ID']}" class="dropdown-item editWindow text-primary">
                                ...
                                <li>
                                    <a href="javascript:;" data_id="${data['RRS_ID']}" class="dropdown-item text-danger delete-record">
    ...
    function resetForm() {
        ...
        $(`${form_id} input[name='RRS_DISPLAY_SEQUENCE']`).val('1');
    }
    ...
    function addForm() {
        ...
        $('#popModalWindowlabel').html('Add Reservation Slot');
    ...
    function submitForm() {
        ...
        $.ajax({
            url: '<?= base_url('/restaurant/reservation-slot/store') ?>',
            type: "post",
            data: fd,
            processData: false,
            contentType: false,
    ...
    $(document).on('click', '.editWindow', function() {
        ...
        var reservation_slot_id = $(this).attr('data_id');

        $(`${form_id} input[name='RRS_ID']`).val(reservation_slot_id);

        $('#popModalWindowlabel').html('Edit Reservation Slot');
        $('#popModalWindow').modal('show');

        var url = '<?php echo base_url('/restaurant/reservation-slot/edit') ?>';
        $.ajax({
            url: url,
            type: "post",
            data: {
    ...
    $(document).on('click', '.delete-record', function() {
        ...
        bootbox.confirm({
            ...
            callback: function(result) {
                if (result) {
                    $.ajax({
                        url: '<?php echo base_url('/restaurant/reservation-slot/delete') ?>',
                        type: "post",
                        data: {
                            id: id,
                            '_method': 'delete'
