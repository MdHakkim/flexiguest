...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="breadcrumb-wrapper py-3 mb-4"><span class="text-muted fw-light">Restaurant /</span> Tables</h4>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="submit-form" class="needs-validation" novalidate>
                        <div class="row g-3">
                            <input type="hidden" name="RT_ID" class="form-control" />

                            <div class="col-md-12">
                                <label class="form-label"><b>Restaurants *</b></label>
                                <select name="RT_RESTAURANT_ID" class="select2 form-select">
                            ...
                            <div class="col-md-6">
                                <label class="form-label"><b>Table No *</b></label>
                                <input type="text" name="RT_TABLE_NO" class="form-control" placeholder="Table No" required />
                            </div>

                            <div class="col-md-6">
                                <label class="form-label"><b>No of Seats *</b></label>
                                <input type="text" name="RT_NO_OF_SEATS" class="form-control" placeholder="No of Seats" required />
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#dataTable_view').DataTable({
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': '<?php echo base_url('/restaurant/table/all-tables') ?>'
            },
            'columns': [{
                    data: ''
                },
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        <div class="d-inline-block">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    <a href="javascript:;" data_id="${data['RT_ID']}" class="dropdown-item editWindow text-primary">
                                ...
                                <li>
                                    <a href="javascript:;" data_id="${data['RT_ID']}" class="dropdown-item text-danger delete-record">
            ...
            responsive: {
                details: {
                    display: $.fn.dataTable.Responsive.display.modal({
                        header: function(row) {
                            var data = row.data();
                            return 'Details of ' + data['RT_TABLE_NO'];
                        }
                    }),
                    type: 'column',
    ...
    function addForm() {
        ...
        $('#popModalWindowlabel').html('Add Table');
    ...
    function submitForm() {
        ...
        $.ajax({
            url: '<?= base_url('/restaurant/table/store') ?>',
            type: "post",
            data: fd,
            processData: false,
            contentType: false,
    ...
    $(document).on('click', '.editWindow', function() {
        ...
        var restaurant_table_id = $(this).attr('data_id');

        $(`${form_id} input[name='RT_ID']`).val(restaurant_table_id);

        $('#popModalWindowlabel').html('Edit Table');
        $('#popModalWindow').modal('show');

        var url = '<?php echo base_url('/restaurant/table/edit') ?>';
        $.ajax({
            url: url,
            type: "post",
            data: {
    ...
    $(document).on('click', '.delete-record', function() {
        ...
        bootbox.confirm({
            ...
            callback: function(result) {
                if (result) {
                    $.ajax({
                        url: '<?php echo base_url('/restaurant/table/delete') ?>',
                        type: "post",
                        data: {
                            id: id,
                            '_method': 'delete'
