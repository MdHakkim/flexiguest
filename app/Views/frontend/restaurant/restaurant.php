...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="breadcrumb-wrapper py-3 mb-4"><span class="text-muted fw-light">Masters /</span> Restaurants</h4>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#dataTable_view').DataTable({
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': '<?php echo base_url('/restaurant/all-restaurant') ?>'
            },
            'columns': [{
                    data: ''
                },
    ...
    $(document).on('click', '.editWindow', function() {
        ...
        var restaurant_id = $(this).attr('data_id');

        $(`${form_id} input[name='id']`).val(restaurant_id);

        $('#popModalWindowlabel').html('Edit Restaurant');
        $('#popModalWindow').modal('show');

        var url = '<?php echo base_url('/restaurant/edit-restaurant') ?>';
        $.ajax({
            url: url,
            type: "post",
            data: {
    ...
    $(document).on('click', '.delete-record', function() {
        ...
        bootbox.confirm({
            ...
            callback: function(result) {
                if (result) {
                    $.ajax({
                        url: '<?php echo base_url('/restaurant/delete-restaurant') ?>',
                        type: "post",
                        data: {
                            id: id,
                            '_method': 'delete'
