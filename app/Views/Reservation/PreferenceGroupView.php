...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="breadcrumb-wrapper py-3 mb-4"><span class="text-muted fw-light">Masters /</span> Preference
            Groups</h4>
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="popModalWindowlabel">Preference Group</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-lable="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="submitForm" class="needs-validation" novalidate>
                        <div class="row g-3">
                            <input type="hidden" name="PF_GR_ID" id="PF_GR_ID" class="form-control" />
...
$(document).ready(function() {
    ...
    $('#dataTable_view').DataTable({
        ...
        'serverMethod': 'post',
        'ajax': {
            'url': '<?php echo base_url('/preferenceGroupView')?>'
        },
        'columns': [{
                data: ''
            },
            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    return (
                        ...
                        '<li><a href="javascript:;" data_sysid="' + data['PF_GR_ID'] +
        ...
        responsive: {
            details: {
                display: $.fn.dataTable.Responsive.display.modal({
                    header: function(row) {
                        var data = row.data();
                        return 'Details of ' + data['PF_GR_CODE'];
                    }
                }),
                type: 'column',
...
// Show Add Preference Group Form
...
function addForm() {
    ...
    $('#popModalWindowlabel').html('Add New Preference Group');
...
$(document).on('click', '.delete-record', function() {
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url: '<?php echo base_url('/deletePreferenceGroup')?>',
                    type: "post",
                    data: {
                        sysid: sysid
                    },
                    ...
                    success: function(respn) {
                        showModalAlert('warning',
                            '<li>The Preference Group has been deleted</li>');
...
// Show Edit Preference Group Form
...
$(document).on('click', '.editWindow', function() {
    ...
    var sysid = $(this).attr('data_sysid');
    $('#popModalWindowlabel').html('Edit Preference Group');
    $("#PF_GR_CODE").prop("readonly", true);

    $('#popModalWindow').modal('show');

    var url = '<?php echo base_url('/editPreferenceGroup')?>';
    $.ajax({
        url: url,
        type: "post",
        headers: {
...
// Add New or Edit Preference Group submit Function

function submitForm(id) {
    hideModalAlerts();
    var formSerialization = $('#' + id).serializeArray();
    var url = '<?php echo base_url('/insertPreferenceGroup')?>';
    $.ajax({
        url: url,
        type: "post",
        ...
        success: function(respn) {
            ...
            if (response != '1') {
                ...
                var alertText = $('#PF_GR_ID').val() == '' ? '<li>The new Preference Group \'' + $(
                        '#PF_GR_CODE')
                    .val() + '\' has been created</li>' : '<li>The Preference Group \'' + $(
                        '#PF_GR_CODE').val() +
                    '\' has been updated</li>';
