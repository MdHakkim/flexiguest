...
<!-- Content wrapper -->
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="breadcrumb-wrapper py-3 mb-4"><span class="text-muted fw-light">Reservations /</span> Sections</h4>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#dataTable_view').DataTable({
        ...
        'serverMethod': 'post',
        'ajax': {
            'url':'<?php echo base_url('/sectionView')?>'
        },
        'columns': [
          { data: 'SC_FL_CODE' },
          { data: 'SC_FL_DESC' },
          { data: 'SC_FL_TARGET_CREDIT' },
          { data: null , render : function ( data, type, row, meta ) {
            return (
              '<div class="d-inline-block">' +
                '<a href="javascript:;" class="btn btn-sm btn-icon dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></a>' +
                '<ul class="dropdown-menu dropdown-menu-end">' +
                  '<li><a href="javascript:;" data_sysid="'+data['SC_FL_ID']+'" class="dropdown-item editWindow">Edit</a></li>' +
                  '<div class="dropdown-divider"></div>' +
                  '<li><a href="javascript:;" data_sysid="'+data['SC_FL_ID']+'" class="dropdown-item text-danger delete-record">Delete</a></li>' +
                '</ul>' +
              '</div>'
            );
          }},
    ...
    var sysid = $(this).attr('data_sysid');
      bootbox.confirm({
        ...
        callback: function (result) {
            if(result){
              $.ajax({
                url: '<?php echo base_url('/deleteSection')?>',
                type: "post",
                data: {sysid:sysid},
  ...
  $(document).on('click','.editWindow',function(){
    var sysid = $(this).attr('data_sysid');
    $('#popModalWindow').modal('show');
    var url = '<?php echo base_url('/editSection')?>';
    $.ajax({
        url: url,
        type: "post",
        headers: {'X-Requested-With': 'XMLHttpRequest'},
  ...
  function submitForm(id){
    ...
    var url = '<?php echo base_url('/insertSection')?>';
    $.ajax({
        url: url,
        type: "post",
