...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="breadcrumb-wrapper py-3 mb-4"><span class="text-muted fw-light">Masters /</span> Item Class</h4>
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="popModalWindowlabel">Item Class</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-lable="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="submitForm" class="needs-validation" novalidate>
                        <div class="row g-3">
                            <input type="hidden" name="IT_CL_ID" id="IT_CL_ID" class="form-control" />

                            <div class="col-md-7">
                                <lable class="form-lable"><b>Class Code *</b></lable>
                                <input type="text" name="IT_CL_CODE" id="IT_CL_CODE" class="form-control bootstrap-maxlength"
                                    maxlength="10" placeholder="eg: HSK" required />
                            </div>
                            <div class="col-md-8">
                                <lable class="form-lable"><b>Description *</b></lable>
                                <input type="text" name="IT_CL_DESC" id="IT_CL_DESC" class="form-control bootstrap-maxlength"
                                    maxlength="50" placeholder="eg: House Keeping" required />
                            </div>
                            ...
                            <div class="col-md-4">
                                <lable class="form-lable"><b>Seq No:</b></lable>
                                <input type="number" name="IT_CL_DIS_SEQ" id="IT_CL_DIS_SEQ" class="form-control" min="1"
                                    placeholder="eg: 2" />
                            </div>
...
$(document).ready(function() {
    ...
    $('#dataTable_view').DataTable({
        ...
        'serverMethod': 'post',
        'ajax': {
            'url': '<?php echo base_url('/ItemClassView')?>'
        },
        'columns': [{
                data: ''
            },
            ...
            
            {
                ...
                render: function(data, type, row, meta) {
                    return (
                        ...
                        '<li><a href="javascript:;" data_sysid="' + data['IT_CL_ID'] +
        ...
        responsive: {
            details: {
                display: $.fn.dataTable.Responsive.display.modal({
                    header: function(row) {
                        var data = row.data();
                        return 'Details of ' + data['DEPT_CODE'];
                    }
                }),
                type: 'column',
...
function addForm() {
    ...
    $('#popModalWindowlabel').html('Add New Item Class');
    $("#IT_CL_CODE").prop("readonly", false);
...
$(document).on('click', '.delete-record', function() {
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url: '<?php echo base_url('/deleteItemClass')?>',
                    type: "post",
                    data: {
                        sysid: sysid
                    },
                    ...
                    success: function(respn) {
                        showModalAlert('warning',
                            '<li>The Item Class has been deleted</li>');
...
$(document).on('click', '.editWindow', function() {
    ...
    var sysid = $(this).attr('data_sysid');
    $('#popModalWindowlabel').html('Edit Item Class');
    $("#IT_CL_CODE").prop("readonly", true);

    $('#popModalWindow').modal('show');

    var url = '<?php echo base_url('/editItemClass')?>';
    $.ajax({
        url: url,
        type: "post",
        headers: {
        ...
        success: function(respn) {
            $(respn).each(function(inx, data) {
                $.each(data, function(fields, datavals) {
                    ...
                    if (field == 'IT_CL_DEPARTMENTS' || field == 'IT_CL_DEPARTMENTS') {
...
function submitForm(id) {
    ...
    var url = '<?php echo base_url('/insertItemClass')?>';
    $.ajax({
        url: url,
        type: "post",
        ...
        success: function(respn) {
            ...
            if (response != '1') {
                ...
                var alertText = $('#IT_CL_ID').val() == '' ? '<li>The new Item Class \'' + $(
                        '#IT_CL_CODE')
                    .val() + '\' has been created</li>' : '<li>The Item Class \'' + $(
                        '#IT_CL_CODE').val() +
                    '\' has been updated</li>';
