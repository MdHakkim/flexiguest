...
<script>
    ...
    function generaListImage(listImage) {
        ...
        $.each(listImage, function(inx, data) {
            if ($.isNumeric(inx)) {
                inx == 0 ? $("#cropped_img").attr('src', '<?php echo $folderPath; ?>' + '/UserDocuments/proof/' + data['DOC_FILE_PATH']) : '';
                var activeClass = (inx == 0 ? 'activeLink linkImg' : 'linkImg');
                liList += '<li data-file_name="' + data['DOC_FILE_PATH'] + '" data_id="' + data['DOC_ID'] + '" class="' + activeClass + '"><img id="imagesmall" class="card-img-top" src="<?php echo $folderPath; ?>' + '/UserDocuments/proof/' + data['DOC_FILE_PATH'] + '"></li>';
            }
        });
